      ...
      <View style={[styles.header, { borderBottomColor: colors.border, backgroundColor: colors.bgCard }]}>
        <View style={styles.headerTop}>
          ...
          <View style={styles.headerActions}>
            ...
            <View style={[styles.viewToggle, { backgroundColor: darkMode ? colors.bgCard : colors.bgTertiary }]}>
              <TouchableOpacity
                style={[styles.viewToggleBtn, viewMode === 'card' && { backgroundColor: colors.primary }]}
                onPress={() => setViewMode('card')}
              >
                <Ionicons name="card-outline" size={16} color={viewMode === 'card' ? '#FFF' : colors.textMuted} />
              </TouchableOpacity>
              <TouchableOpacity
      ...
      <ScrollView style={styles.content} contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.cardsList}>
          {(displayCards || []).length === 0 ? (
            ...
            // ═══════ MODO LISTA COMPACTO ═══════
            (displayCards || []).map(card => {
              const { used, available, percentage, availablePercentage } = getCardProgress(card);
              const progressColor = getProgressColor(availablePercentage);
              const gradientColors = getCardGradientColors(card.gradientClass);
              const pendingCount = (getCardPendingInvoices(card.id) || []).length;
              return (
                <TouchableOpacity
                  key={card.id}
                  style={[styles.listCardItem, { backgroundColor: darkMode ? colors.bgCard : colors.bgTertiary }]}
                  onPress={() => openCardDetail(card)}
                  onLongPress={() => canEditCard(card) && handleDeleteCard(card.id)}
                  activeOpacity={0.85}
                >
                  <View style={styles.listCardRow}>
                    {/* Quadrado colorido do cartão */}
                    <View style={styles.listCardLeft}>
                      <LinearGradient
                        colors={gradientColors}
                        style={styles.listCardColorBox}
                        start={{ x: 0, y: 0 }}
                        end={{ x: 1, y: 1 }}
                      >
                    ...
                    <View style={styles.listCardInfo}>
                      <View style={styles.listCardHeader}>
                        <Text style={[styles.listCardName, { color: colors.textPrimary }]} numberOfLines={1}>{card.name}</Text>
                        <Text style={[styles.listCardNumber, { color: colors.textMuted }]}>{card.number}</Text>
                      </View>
                      <View style={styles.listCardMeta}>
                        <Text style={[styles.listCardLimit, { color: colors.textMuted }]}>
                          {t('cards.available')}: <Text style={{ color: progressColor, fontWeight: '700' }}>{formatCurrency(available)}</Text>
                        </Text>
                        <Text style={[styles.listCardUsed, { color: colors.textMuted }]}>
                          {t('cards.used')}: <Text style={{ color: colors.danger, fontWeight: '700' }}>{formatCurrency(used)}</Text>
                        </Text>
                      </View>
                    </View>
                    ...
                    <View style={styles.listCardRight}>
                      {pendingCount > 0 && (
                        <View style={[styles.listInvoiceBadge, { backgroundColor: colors.danger }]}>
                          <Text style={styles.listInvoiceBadgeText}>{pendingCount}</Text>
                        </View>
                      )}
                  ...
                  <View style={styles.listProgressBar}>
                    <View style={[styles.listProgressTrack, { backgroundColor: darkMode ? colors.bgTertiary : colors.border }]}>
                      <View style={[styles.listProgressFill, { width: `${Math.min(percentage, 100)}%`, backgroundColor: progressColor }]} />
                    </View>
                    <Text style={[styles.listProgressPercent, { color: progressColor }]}>{percentage.toFixed(0)}%</Text>
                  </View>

                  {/* Badges de compartilhamento (linha inferior) */}
...
const styles = StyleSheet.create({
  ...
  // ═══════ ESTILOS DO MODO LISTA COMPACTO ═══════
  headerActions: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  viewToggle: { flexDirection: 'row', alignItems: 'center', padding: 2, borderRadius: 10, gap: 2 },
  viewToggleBtn: { padding: 6, borderRadius: 8 },
  listCardItem: { borderRadius: 16, padding: 14, marginBottom: 10 },
  listCardRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  listCardLeft: { justifyContent: 'center' },
  listCardColorBox: { width: 48, height: 48, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  listCardInfo: { flex: 1, justifyContent: 'center' },
  listCardHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 },
  listCardName: { fontSize: 15, fontWeight: '700', flex: 1 },
  listCardNumber: { fontSize: 12, fontWeight: '500' },
  listCardMeta: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  listCardLimit: { fontSize: 12, fontWeight: '500' },
  listCardUsed: { fontSize: 12, fontWeight: '500' },
  listCardRight: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  listInvoiceBadge: { minWidth: 22, height: 22, borderRadius: 11, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 6 },
  listInvoiceBadgeText: { color: '#FFF', fontSize: 11, fontWeight: '700' },
  listProgressBar: { flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 10 },
  listProgressTrack: { flex: 1, height: 6, borderRadius: 3, overflow: 'hidden' },
  listProgressFill: { height: '100%', borderRadius: 3 },
  listProgressPercent: { fontSize: 11, fontWeight: '700', minWidth: 32, textAlign: 'right' },
  listCardBadges: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 8, flexWrap: 'wrap' },
});

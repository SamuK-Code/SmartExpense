        ...
        <View style={styles.hubHeaderTop}>
          <View>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              ...
              <View style={[
                styles.statusBadge,
                { backgroundColor: isSupabaseOnline ? colors.success + '20' : colors.danger + '20' }
              ]}>
                <View style={[
                  styles.statusDot,
                  { backgroundColor: isSupabaseOnline ? colors.success : colors.danger }
                ]} />
                <Text style={[
                  styles.statusText,
                  { color: isSupabaseOnline ? colors.success : colors.danger }
                ]}>
                  {isSupabaseOnline ? 'Online' : 'Offline'}
                </Text>
              </View>
            </View>
            ...
            {currentCircle && (
              <View style={[styles.currentCircleCard, { backgroundColor: colors.bgCard }]}>
                ...
                {offlineQueue.length > 0 && (
                  <View style={[styles.offlineQueueBox, { backgroundColor: colors.warning + '15' }]}>
                    <View style={styles.offlineQueueRow}>
                      <Ionicons name="cloud-offline-outline" size={16} color={colors.warning} />
                      <Text style={[styles.offlineQueueText, { color: colors.warning }]}>
                        {isProcessingQueue ? 'Sincronizando...' : `${offlineQueue.length} operação(ões) pendente(s)`}
                      </Text>
                    </View>
                    {isProcessingQueue && (
...
const styles = StyleSheet.create({
  ...
  // Offline Queue
  offlineQueueBox: { borderRadius: 10, padding: 12, marginBottom: 12, marginTop: 4 },
  offlineQueueRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  offlineQueueText: { fontSize: 13, fontWeight: '600' },
});
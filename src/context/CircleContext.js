...
export const CircleProvider = ({ children }) => {
  ...
  const STORAGE_KEYS = {
    ...
    offlineQueue: 'circle_offline_queue',
  };
  ...
  const addToOfflineQueue = async (operation) => {
    const newQueue = [...offlineQueue, { ...operation, timestamp: Date.now() }];
    setOfflineQueue(newQueue);
    await saveToStorage(STORAGE_KEYS.offlineQueue, newQueue);
  };

  const removeFromOfflineQueue = async (index) => {
  ...
  const clearOfflineQueue = async () => {
    setOfflineQueue([]);
    await saveToStorage(STORAGE_KEYS.offlineQueue, []);
  };

  const processOfflineQueue = async () => {
    if (isProcessingQueue || offlineQueue.length === 0 || !isSupabaseOnline) return;
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      try {
        switch (op.type) {
          case 'share_item':
            await shareItem(op.itemType, op.itemData, op.permissions);
            break;
          case 'unshare_item':
            await unshareItem(op.itemType, op.itemId);
            break;
          case 'create_circle':
            await createCircle(op.name);
            break;
          case 'join_circle':
            await joinCircle(op.code);
            break;
          case 'leave_circle':
            await leaveCircle(op.circleId);
            break;
          case 'update_circle_name':
            await updateCircleName(op.circleId, op.newName);
            break;
    ...
    await saveToStorage(STORAGE_KEYS.offlineQueue, queue);
  ...
  const safeSupabaseCall = async (operation, offlineFallback = null) => {
    const online = await checkSupabaseHealth();
    if (!online) {
      if (offlineFallback) {
        await addToOfflineQueue(offlineFallback);
      }
      return { data: null, error: 'offline', offline: true };
    }
    try {
  ...
  useEffect(() => {
    ...
    if (isSupabaseOnline && currentCircle && currentUser) {
      startRealTimeSubscriptions();
      syncWithCircle();
    }
  }, [isSupabaseOnline]);
  ...
  }, [currentCircle?.id, isSupabaseOnline]);
  ...
  const register = async (username, password) => {
    ...
    try {
      const result = await safeSupabaseCall(
        ...
        { type: 'register', username: cleanUsername, password }
      );

      if (result?.offline) {
        return { error: 'Servidor indisponível. Operação salva para sincronizar depois.' };
      }
      return result;
    } catch (e) {
  ...
  const login = async (username, password) => {
    ...
    try {
      const result = await safeSupabaseCall(
        ...
        { type: 'login', username: cleanUsername, password }
      );

      if (result?.offline) {
        return { error: 'Servidor indisponível. Verifique sua conexão.' };
      }
      return result;
    } catch (e) {
  ...
  const createCircle = async (name) => {
    ...
    try {
      const result = await safeSupabaseCall(
        ...
        { type: 'create_circle', name: cleanName }
      );

      if (result?.offline) {
        return { error: 'Servidor indisponível. Círculo será criado quando voltar online.' };
      }
      return result;
    } catch (e) {
  ...
  const joinCircle = async (code) => {
    ...
    try {
      const result = await safeSupabaseCall(
        ...
        { type: 'join_circle', code: cleanCode }
      );

      if (result?.offline) {
        return { error: 'Servidor indisponível. Entrada será processada depois.' };
      }
      return result;
    } catch (e) {
  ...
  const leaveCircle = async (circleId = null) => {
    ...
    try {
      const result = await safeSupabaseCall(
        ...
        { type: 'leave_circle', circleId: targetCircleId }
      );
      ...
      if (result?.offline) {
        return { success: true, warning: 'Saída será sincronizada quando voltar online.' };
      }
  ...
  const generateNewInviteCode = async () => {
    ...
    try {
      ...
      if (result?.offline) {
        return { error: 'Servidor indisponível. Código será gerado depois.' };
      }
      return result;
    } catch (e) {
  ...
  const updateCircleName = async (circleId, newName) => {
    ...
    try {
      const result = await safeSupabaseCall(
        ...
        { type: 'update_circle_name', circleId, newName: cleanName }
      );

      if (result?.offline) {
        return { error: 'Servidor indisponível. Nome será atualizado depois.' };
      }
      return result;
    } catch (e) {
  ...
  const shareItem = async (itemType, itemData, permissions = { view: true, edit: false }) => {
    ...
    try {
      const result = await safeSupabaseCall(
        ...
        { type: 'share_item', itemType: cleanType, itemData, permissions }
      );

      if (result?.offline) {
        return { error: 'Servidor indisponível. Item será compartilhado depois.' };
      }
      return result;
    } catch (e) {
  ...
  const unshareItem = async (itemType, itemId) => {
    ...
    try {
      const result = await safeSupabaseCall(
        ...
        { type: 'unshare_item', itemType: cleanType, itemId: cleanId }
      );
      ...
      if (result?.offline) {
        return { success: true, warning: 'Remoção será sincronizada depois.' };
      }
      return result || { success: true };
    } catch (e) {
  ...
  const shareAllItems = async (itemType, itemsList, permissions = { view: true, edit: false }) => {
    ...
    try {
      const result = await safeSupabaseCall(
        ...
        { type: 'share_all', itemType, itemsList, permissions }
      );
      ...
      if (result?.offline) {
        return { error: 'Servidor indisponível. Itens serão compartilhados depois.' };
      }
      return result;
    } catch (e) {
  ...
  const logActivity = async (action, itemType, itemName, itemId) => {
    ...
    try {
      const result = await safeSupabaseCall(
        ...
        { type: 'log_activity', action, itemType, itemName, itemId }
      );
    } catch (e) {
